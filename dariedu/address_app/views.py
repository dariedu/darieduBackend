...
class RouteSheetViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_staff:
            return RouteSheet.objects.filter(location__curator=self.request.user)
        else:
            return RouteSheet.objects.filter(user=self.request.user, assignments__delivery__in_execution=True)
    ...
    def assign(self, request, pk=None):
        ...
        if self.request.user.is_staff:
            ...
            volunteers = routesheet.delivery_route.delivery.assignment.volunteer.all()
            if routesheet.delivery_route.delivery.is_active:
                try:
                    user = User.get(pk=pk)
