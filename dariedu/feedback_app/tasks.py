...
def export_to_google_feedback_user(feedback_id, user_id):
    logger.info(f"Exporting feedback {feedback_id} for user {user_id}")
    try:
        ...
        logger.debug(f"User  data: {data_user}")
        ...
        if data_user in values_list:
            ...
            logger.info(f"Updated existing user data for {data_user} in row {first_empty_row + 1}")
        else:
            ...
            logger.info(f"Added new user data for {data_user} in column {next_column}")
        ...
        logger.error(f"Error exporting feedback {feedback_id} for user {user_id}: {str(e)}")


@shared_task
def export_to_google_request_message(request_id, user_id):

    logger.info(f"Exporting request {request_id} for user {user_id}")
    try:
        tg_id = User.objects.get(id=user_id).tg_id
        logger.debug(f"User data: {tg_id}")
        ...
        existing_datas = worksheet.get_all_records()
        first_row_values = cache.get(settings.FIRST_ROW_VALUES_CACHE_KEY)
        logger.debug(f"Existing data: {existing_datas}")
        ...
        if user_row_index is not None:
            ...
            logger.info(f"Updated existing user data for {tg_id} in row {user_row_index}")
    except Exception as e:
        logger.error(f"Error exporting request {request_id} for user {user_id}: {str(e)}")
