...
#             # Можно вернуть ответ с ошибкой, если это необходимо
#             response = self.handle_error(e)
#
#         # Логируем SQL-запросы
#         self.log_sql_queries()
#
#         return response
#
#     def log_request(self, request):
...
#         except Exception as e:
#             self.logger.error(f"Error logging request body: {e}", exc_info=True)
#
#         self.logger.info(f"Request Method: {request.method}")
#         self.logger.info(f"Request Path: {request.get_full_path()}")
...
#         duration = time.time() - start_time
#         self.logger.info(f"Response: {response.status_code} {request.method} {request.get_full_path()} - Duration: {duration:.2f}s")
...
#         except Exception as e:
#             self.logger.error(f"Error logging response body: {e}", exc_info=True)
#
...
#         for query in connection.queries:
#             self.logger.debug(f"SQL Query: {query['sql']} - Time: {query['time']}s")

 # заголовки запросов и ответов, или обрабатывать исключения, если они возникают